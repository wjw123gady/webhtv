...
| `armeabi-v7a` | 已验证二进制 `0.41.0-556-g9ce79bcaa` | `38b88335f99e`（8.1.2） | `7.362.0` / `82224764a981` | 使用相同兼容策略，保留独立 32 位产物 |
...
| FFmpeg | `38b88335f99e76ed89ff3c93f877fdefce736c13`（8.1.2 动态库） |
...
1. 常规升级要求 `libmpv.so`、FFmpeg 七个动态库和 `libc++_shared.so` 按 ABI 成套更新。当前仓库是经过 Android 15 真机验证的临时兼容例外：恢复升级前 `libmpv.so`，保留 FFmpeg 8.1.2 动态库。不要把该例外推广到其他版本组合。
...
已知兼容性限制：使用当前升级后的 FFmpeg 8.1.2 头文件/构建环境重新生成 `libmpv.so`，会在 vivo Android 15 真机播放初始化阶段触发 destroyed-mutex native 崩溃。恢复升级前已验证 `libmpv.so` 后，同一视频可正常播放。因此 `third_party/mpv-native-lock.json` 当前是继续定位兼容组合的源码实验基线，不能视为已提交热修复 assets 的完全可复现描述。
...
候选输出完成 Android 15 真机回归后，才允许同步安装两套 ABI 和 JNI 桥：
...
脚本读取 `third_party/mpv-native-lock.json`，自动应用 MPV 光盘控制补丁；所有下载和中间文件放在被 Git 忽略的 `build/mpv-native/`。在兼容组合定位完成前，不要直接使用 `--install` 覆盖已验证 assets。Gradle 和 GitHub Actions 始终复用仓库已提交的热修复 `.so`。完整排查记录见本地 `plans/MPV原生依赖升级与Android崩溃排查记录.md`。

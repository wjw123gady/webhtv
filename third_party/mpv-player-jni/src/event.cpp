...
{
    int reason = end_file ? (int)end_file->reason : -1;
    int error = end_file ? end_file->error : 0;
    const char *text = error ? mpv_error_string(error) : "";
    jstring jtext = env->NewStringUTF(text);
    env->CallStaticVoidMethod(mpv_MPVLib, mpv_MPVLib_endFile_IIS,
        (jint) reason, (jint) error, jtext);
    if (jtext)
